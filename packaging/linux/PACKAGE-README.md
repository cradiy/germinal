...
directory on the system prefix, for example `/usr/local`. Native RPM and DEB packages install the
same files under `/usr` and declare the direct runtime library dependencies.

The `linux-musl` archive is built for musl but is not fully static. It requires musl-compatible
Fontconfig, FreeType, GLib, GStreamer, and GStreamer plugins on the destination system. DEB and RPM
outputs intentionally remain glibc packages.
