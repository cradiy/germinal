    ...
    fn ground_dispatch<P: Perform>(performer: &mut P, text: &str) {
        if text.is_ascii() {
            ...
            for (index, byte) in bytes.iter().copied().enumerate() {
                if byte <= 0x1f {
                    if printable_start != index {
                        performer.print_str(&text[printable_start..index]);
                    }
                    performer.execute(byte);
                    printable_start = index + 1;
                }
            }

            if printable_start != text.len() {
                performer.print_str(&text[printable_start..]);
            }
            return;
        }
        ...
        for (index, c) in text.char_indices() {
            if matches!(c, '\x00'..='\x1f' | '\u{80}'..='\u{9f}') {
                ...
                printable_start = index + c.len_utf8();
            }
        }
...
pub trait Perform {
    ...
    /// Draw a sequence of printable characters to the screen and update states.
    ///
    /// The default implementation preserves the per-character behavior for
    /// performers which do not provide a more efficient batch operation.
