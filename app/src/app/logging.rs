...
pub fn init_logging(config: &LoggingConfig, paths: &AppPaths) -> AppResult<()> {
	...
	let log_file = OpenOptions::new()
		...
		.append(true)
		.open(&log_file_path)
		.map_err(|source| AppError::OpenLogFile { path: log_file_path.clone(), source })?;
	let file_writer = SharedFileWriter::new(log_file);

	tracing_subscriber::registry()
		.with(fmt::layer().with_writer(io::stderr).with_filter(level_filter(config.console_level)))
		.with(
			fmt::layer()
				.with_ansi(false)
...
impl SharedFileWriter {
	fn new(file: std::fs::File) -> Self { Self { file: Arc::new(Mutex::new(file)) } }
}
