...
impl HostPressedModifiers {
	fn control_key(self) -> bool { self.left_control || self.right_control }
...
impl HostPasteController {
	pub fn set_modifiers(&mut self, modifiers: HostPasteModifiers) { self.modifiers = modifiers; }
	...
	pub fn handle_shortcut(
		...
		Ok(HostPasteAction::Dispatch(GShellInput { gshell_id, event: GShellInputEvent::Paste(text) }))
	}
...
fn matches_paste_shortcut(
	...
	if !matches!(state, WindowInputElementState::Pressed | WindowInputElementState::Released) {
		return false;
	}
...
mod tests {
	...
	fn ctrl_shift_v_pressed_requests_paste_dispatch() {
		assert!(matches_paste_shortcut(
			HostPasteModifiers { control: true, shift: true },
	...
	fn ctrl_v_without_shift_does_not_trigger_host_paste() {
		assert!(!matches_paste_shortcut(
			HostPasteModifiers { control: true, shift: false },
			WindowInputElementState::Pressed,
			&WindowInputKey::Character("v".into()),
			PhysicalKey::Code(KeyCode::KeyV),
		));
	...
	fn tracked_physical_modifiers_trigger_paste_without_modifiers_changed_event() {
		...
		controller
			.observe_key_event(WindowInputElementState::Pressed, PhysicalKey::Code(KeyCode::ControlLeft));
		controller
			.observe_key_event(WindowInputElementState::Pressed, PhysicalKey::Code(KeyCode::ShiftLeft));
