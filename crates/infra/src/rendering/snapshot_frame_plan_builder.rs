...
impl<P> SnapshotFramePlanBuilder<P> {
    pub fn new(snapshot_provider: P) -> Self {
    ...
    P: TerminalSnapshotProvider,
{
    fn build(&self, task: BuildFramePlanTask) -> BuiltFramePlan {
        ...
        match self
            .snapshot_provider
            .snapshot_for_build(task.target_id, task.seq)
        {
            Some(snapshot) => {
...
fn append_snapshot_commands(commands: &mut Vec<RenderCommandDto>, snapshot: TerminalSnapshot) {
    let line_by_row: BTreeMap<u32, String> = snapshot
        .lines
        .into_iter()
        .map(|line| (line.row, line.text))
        .collect();
...
mod tests {
    ...
    fn builds_frame_only_from_dirty_terminal_snapshot_rows() {
        ...
        let frame = builder.build(BuildFramePlanTask {
