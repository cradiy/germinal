...
impl WgpuImageSurfaceRenderer {
	...
	pub fn remove_render_target(&self, target_id: RenderTargetId) {
		self.inner.borrow_mut().textures.retain(|(target, _), _| *target != target_id);
	}
	...
	pub(crate) fn encode_layer(
		...
		let WgpuImageEncodeContext { device, encoder, target_view, color_format, plan, load_op } =
			context;
		...
		let color_attachments = [Some(wgpu::RenderPassColorAttachment {
			...
			ops: wgpu::Operations {
				load: load_op,
				store: if plan.store { wgpu::StoreOp::Store } else { wgpu::StoreOp::Discard },
			},
		})];
		...
		for (index, surface) in surfaces.iter().enumerate() {
			let Some(texture) = inner.textures.get(&(prepared.target_id, surface.generation)) else {
				continue;
			};
	...
	fn sync_textures(
		...
		for image in &snapshot.image_surfaces {
			...
			inner.textures.entry(key).or_insert_with(|| upload_texture(device, queue, image));
		}
	}
}
...
impl WgpuImagePreparedFrame {
	pub fn is_empty(&self) -> bool { self.below_text.is_empty() && self.above_text.is_empty() }
}
...
impl WgpuImageRenderResult {
	fn empty(target_id: RenderTargetId, seq: Seq) -> Self {
		Self { target_id, seq, encoded: false, draw_count: 0, surface_count: 0 }
	}
}
...
fn upload_texture(
	...
	WgpuImageTexture { _texture: texture, view, sampler }
}
...
fn create_vertex_buffer(device: &wgpu::Device, surfaces: &[WgpuImagePrepared]) -> wgpu::Buffer {
	let mut bytes = Vec::with_capacity(surfaces.len() * IMAGE_VERTEX_COUNT as usize * WgpuImageVertex::BYTE_SIZE);
...
fn vertices_for_image(
	...
	let content_right = config.content_origin_x.saturating_add(config.content_width_px) as f32;
	let content_bottom = config.content_origin_y.saturating_add(config.content_height_px) as f32;
	...
	Some([
		WgpuImageVertex { position_ndc: [left, top], uv: [u0, v0] },
		WgpuImageVertex { position_ndc: [right, top], uv: [u1, v0] },
		WgpuImageVertex { position_ndc: [right, bottom], uv: [u1, v1] },
		WgpuImageVertex { position_ndc: [left, top], uv: [u0, v0] },
		WgpuImageVertex { position_ndc: [right, bottom], uv: [u1, v1] },
		WgpuImageVertex { position_ndc: [left, bottom], uv: [u0, v1] },
	])
}
