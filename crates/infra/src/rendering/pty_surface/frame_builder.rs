...
impl Default for WgpuTerminalFrameBuilder {
	fn default() -> Self { Self::new(WgpuRendererConfig::default()) }
}
...
impl WgpuTerminalPreparedFrame {
	pub fn quad_count(&self) -> usize { self.vertex_buffer.vertices.len() / 4 }

	pub fn vertex_count(&self) -> usize { self.vertex_buffer.vertices.len() }

	pub fn index_count(&self) -> usize { self.vertex_buffer.indices.len() }

	pub fn has_draw_work(&self) -> bool {
		!self.upload_bytes.is_empty() && self.render_pass_plan.is_some()
	}

	pub fn has_glyph_atlas_work(&self) -> bool { self.glyph_atlas_frame.has_upload_work() }

	pub fn has_mapped_glyph_uvs(&self) -> bool { self.glyph_uv_map_result.mapped_all() }
