...
impl std::fmt::Debug for WgpuWorkspaceDividerRenderer {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("WgpuWorkspaceDividerRenderer").finish_non_exhaustive()
	}
}
...
mod tests {
	...
	fn builds_vertical_divider_for_adjacent_targets() {
		...
		assert_eq!(
			...
			vec![WorkspaceDividerRect { x_px: 49, y_px: 0, width_px: 2, height_px: 40 }]
		);
	}
