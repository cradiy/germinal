...
impl WgpuTerminalGlyphAtlasUploadBytes {
	...
	pub fn expected_byte_len(&self) -> usize { (self.width_px * self.height_px * 4) as usize }

	pub fn bytes_per_row(&self) -> u32 { self.width_px * 4 }

	pub fn rows_per_image(&self) -> u32 { self.height_px }

	pub fn is_tightly_packed(&self) -> bool { self.byte_len() == self.expected_byte_len() }
}
...
fn rgba_pixels_from_atlas(atlas: &WgpuTerminalGlyphAtlas) -> Vec<u8> {
	...
	if atlas.pixels.len() == alpha_len {
		return atlas.pixels.iter().flat_map(|alpha| [0, 0, 0, *alpha]).collect();
	}
...
mod tests {
	...
	fn builds_upload_bytes_from_debug_glyph_atlas() {
		...
		let upload_bytes = factory.build_upload_bytes(&atlas).expect("upload bytes should exist");
