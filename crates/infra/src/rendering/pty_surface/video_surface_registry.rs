...
impl WgpuVideoSurfaceRegistry {
    ...
    pub fn replace_frame(
        ...
        let mut inner = self.inner.borrow_mut();
        let Some(state) = inner.get_mut(&WgpuVideoSurfaceKey {
            target_id,
            id: id.to_string(),
        }) else {
            return false;
...
mod tests {
    ...
    fn sync_snapshot_tracks_and_replaces_target_video_surfaces() {
        ...
        assert_eq!(
            registry
                .registration(target_id, "left")
                .map(|registration| registration.surface.x_px),
            Some(10)
        );
        ...
        assert_eq!(
            registry
                .registration(target_id, "right")
                .map(|registration| registration.latest_seq),
            Some(Seq::new(2))
        );
