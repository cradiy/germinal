...
impl WgpuTerminalGlyphUvMapper {
	...
	pub fn apply_glyph_uvs(
		...
		while index + 3 < vertices.len() {
			...
			let Some(c) = char::from_u32(vertices[index].glyph_codepoint) else {
				missing_vertices += 4;
				index += 4;
				continue;
			};

			let Some(entry) = atlas.entry(c) else {
				missing_vertices += 4;
				index += 4;
				continue;
			};
		...
		WgpuTerminalGlyphUvMapResult { glyph_vertices, mapped_vertices, missing_vertices }
	}
}
...
impl WgpuTerminalGlyphUvMapResult {
	...
	pub fn has_missing(&self) -> bool { self.missing_vertices > 0 }
}
...
mod tests {
	...
	fn maps_glyph_quad_uvs_from_atlas_entry() {
		...
		assert_eq!(result, WgpuTerminalGlyphUvMapResult {
			glyph_vertices:   4,
			mapped_vertices:  4,
			missing_vertices: 0,
		});
		...
		assert_eq!(vertex_buffer.vertices[0].uv, [entry.uv.min_u, entry.uv.min_v]);
		assert_eq!(vertex_buffer.vertices[1].uv, [entry.uv.max_u, entry.uv.min_v]);
		assert_eq!(vertex_buffer.vertices[2].uv, [entry.uv.max_u, entry.uv.max_v]);
		assert_eq!(vertex_buffer.vertices[3].uv, [entry.uv.min_u, entry.uv.max_v]);
	}
