...
impl WgpuTerminalRenderPassPlan {
	pub fn new(draw_plan: WgpuDrawIndexedPlan) -> Self {
		Self {
			commands: vec![
				...
				WgpuRenderPassCommand::SetBindGroup { index: 0, dynamic_offsets: Vec::new() },
				WgpuRenderPassCommand::SetVertexBuffer { slot: draw_plan.vertex_slot },
				WgpuRenderPassCommand::SetIndexBuffer { format: draw_plan.index_format },
	...
	pub fn has_draw_indexed(&self) -> bool {
		self.commands.iter().any(|command| matches!(command, WgpuRenderPassCommand::DrawIndexed { .. }))
	}
}
...
pub enum WgpuRenderPassCommand {
	SetPipeline,
	SetBindGroup { index: u32, dynamic_offsets: Vec<u32> },
...
impl WgpuRenderPassPlanRecorder {
	...
	pub fn clear(&mut self) { self.recorded.clear(); }
}
...
mod tests {
	...
	fn builds_render_pass_plan_from_draw_indexed_plan() {
		...
		assert_eq!(plan.commands[1], WgpuRenderPassCommand::SetBindGroup {
			index:           0,
			dynamic_offsets: Vec::new(),
		});

		assert_eq!(plan.commands[2], WgpuRenderPassCommand::SetVertexBuffer { slot: 0 });

		assert_eq!(plan.commands[3], WgpuRenderPassCommand::SetIndexBuffer {
			format: wgpu::IndexFormat::Uint32,
		});

		assert_eq!(plan.commands[4], WgpuRenderPassCommand::DrawIndexed {
			indices:     0..42,
			base_vertex: 0,
			instances:   0..1,
		});
