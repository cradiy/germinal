...
impl WgpuTerminalCommandPlan {
    pub fn new(
        ...
        if render_target_plan.is_empty() {
            return Self {
                target_id: upload_plan.target_id,
                seq: upload_plan.seq,
                commands,
            };
        }
        ...
        if upload_plan.has_draw_work() {
            commands.extend([
                ...
                WgpuTerminalCommand::SetBindGroup {
                    index: 0,
                    dynamic_offsets_len: 0,
                },
                WgpuTerminalCommand::SetVertexBuffer { slot: 0 },
                WgpuTerminalCommand::SetIndexBuffer {
    ...
    pub fn has_draw_work(&self) -> bool {
        self.commands
            .iter()
            .any(|command| matches!(command, WgpuTerminalCommand::DrawIndexed { .. }))
    }
    ...
    pub fn submit_count(&self) -> usize {
        self.commands
            .iter()
            .filter(|command| matches!(command, WgpuTerminalCommand::Submit))
            .count()
    }
}
...
mod tests {
    ...
    fn builds_command_plan_for_draw_frame() {
        ...
        let command_plan = WgpuTerminalCommandPlan::new(
            WgpuTerminalRenderTargetPlan::new(1280, 720),
            &upload_plan,
        );
        ...
        assert_eq!(
            ...
            vec![
                WgpuTerminalCommand::BeginRenderPass {
                    width_px: 1280,
                    height_px: 720,
                    store: true
                },
    ...
    fn empty_render_target_builds_empty_command_plan() {
        ...
        let vertex_upload = crate::rendering::pty_surface::buffer_uploader::WgpuBufferUploadBytes {
            vertex_data: Arc::from(Vec::<
                crate::rendering::pty_surface::buffer_uploader::WgpuGpuVertex,
            >::new()),
