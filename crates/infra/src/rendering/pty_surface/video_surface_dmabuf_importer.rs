...
pub fn import_nv12_dmabuf_frame(
    ...
    let uv_texture = Arc::new(import_plane_texture(
        ...
        PlaneImportRequest {
            ...
            estimated_plane_size: estimated_plane_size_bytes(
                frame.uv_plane.stride,
                frame.height_px / 2,
            ),
        },
    )?);
...
fn import_plane_texture(
    ...
    let plane_layouts = [subresource_layout_of(
        request.plane,
        request.estimated_plane_size,
    )];
    ...
    let allocation_size = image_requirements.size.max(dmabuf_size).max(
        request
            .plane
            .offset
            .saturating_add(request.estimated_plane_size),
    );
    let mut import_memory_info = vk::ImportMemoryFdInfoKHR::default()
        .handle_type(vk::ExternalMemoryHandleTypeFlags::DMA_BUF_EXT)
        .fd(dup(&*request.plane.fd)
            .map_err(|source| VideoSurfaceImportError::DuplicatePlaneFd { source })?
            .into_raw_fd());
