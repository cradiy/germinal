...
impl WgpuTerminalRenderPassPlanEncoder {
	...
	where E: WgpuTerminalRenderPassEncoder {
		for command in &plan.commands {
			match command {
				WgpuRenderPassCommand::SetPipeline => {
					encoder.set_pipeline();
				}
				WgpuRenderPassCommand::SetBindGroup { index, dynamic_offsets } => {
				...
				WgpuRenderPassCommand::DrawIndexed { indices, base_vertex, instances } => {
...
impl WgpuTerminalRenderPassEncoder for RecordingWgpuTerminalRenderPassEncoder {
	fn set_pipeline(&mut self) { self.calls.push(RecordedWgpuRenderPassCall::SetPipeline); }
	...
	fn set_vertex_buffer(&mut self, slot: u32) {
		self.calls.push(RecordedWgpuRenderPassCall::SetVertexBuffer { slot });
	}

	fn set_index_buffer(&mut self, format: wgpu::IndexFormat) {
		self.calls.push(RecordedWgpuRenderPassCall::SetIndexBuffer { format });
	}
	...
	fn draw_indexed(
		...
		self.calls.push(RecordedWgpuRenderPassCall::DrawIndexed { indices, base_vertex, instances });
	}
}
...
mod tests {
	...
	fn encodes_render_pass_plan_into_encoder_calls() {
		...
		assert_eq!(encoder.calls, vec![
			RecordedWgpuRenderPassCall::SetPipeline,
			RecordedWgpuRenderPassCall::SetBindGroup { index: 0, dynamic_offsets: Vec::new() },
			RecordedWgpuRenderPassCall::SetVertexBuffer { slot: 0 },
			RecordedWgpuRenderPassCall::SetIndexBuffer { format: wgpu::IndexFormat::Uint32 },
