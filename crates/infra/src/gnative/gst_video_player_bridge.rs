...
use germinal_ports::{
	error::BoxResult,
	event::runtime_event::{RuntimeEvent, WorkspaceRuntimeEvent},
};
#[cfg(target_os = "linux")]
use gst::prelude::*;
#[cfg(target_os = "linux")]
...
#[derive(Debug, Error)]
enum GstVideoPlayerBridgeError {
	...
	MissingPlaneMemory { plane_index: usize, memory_index: usize, memory_count: usize },
...
type RuntimeEventCallback = dyn Fn(RuntimeEvent) -> BoxResult<()> + Send + Sync;
...
impl GstVideoPlayerBridge {
	pub fn new(dispatch: Arc<RuntimeEventCallback>) -> BoxResult<Self> {
		#[cfg(target_os = "linux")]
		gst::init()?;
		...
		Ok(Self { command_tx, pending_frames })
	}

	pub fn drain_pending_video_surface_frames(&self) -> Vec<PendingVideoSurfaceFrame> {
		let mut pending_frames = self.pending_frames.lock().expect("pending frames lock poisoned");
...
fn run_playback_runtime(
	...
	while let Ok(command) = command_rx.recv() {
		debug!(?command, "ignored media bridge command on unsupported platform");
	}
}
...
fn run_playback_runtime(
	...
	while let Ok(command) = command_rx.recv() {
		if let Err(error) = runtime.handle_command(command) {
			warn!(error = %error, "media bridge command failed");
		}
	}
...
impl LinuxPlaybackRuntime {
	fn new(
		...
		Self { pending_frames, dispatch, player: None }
	}

	fn handle_command(&mut self, command: PlaybackBridgeCommand) -> BoxResult<()> {
		match command {
			PlaybackBridgeCommand::OpenFile { path, surface_id } => self.open_file(&path, &surface_id),
	...
	fn open_file(&mut self, path: &str, surface_id: &str) -> BoxResult<()> {
		let canonical_path = Path::new(path);
		if !canonical_path.is_file() {
			return Err(GstVideoPlayerBridgeError::UnreadableVideoPath { path: path.to_string() }.into());
		}

		self.stop_current();
		self.clear_pending_frames();

		let uri = gst::glib::filename_to_uri(canonical_path, None)?;
	...
	fn set_state(&self, state: gst::State) -> BoxResult<()> {
		let player = self.player.as_ref().ok_or(GstVideoPlayerBridgeError::NoOpenedVideoToControl)?;
		player.pipeline.set_state(state)?;
		Ok(())
	}

	fn seek(&self, position_us: u64) -> BoxResult<()> {
		let player = self.player.as_ref().ok_or(GstVideoPlayerBridgeError::NoOpenedVideoToSeek)?;
		player.pipeline.seek_simple(
	...
	appsink.set_callbacks(
		gst_app::AppSinkCallbacks::builder()
			...
			.new_preroll({
				...
				move |appsink| handle_new_video_preroll(appsink, &surface_id, &pending_frames, &dispatch)
			})
...
fn queue_video_sample(
	...
	let frame = video_surface_frame_from_sample(&sample).map_err(|error| {
		warn!(error = %error, "failed to import gstreamer video sample");
		gst::FlowError::NotNegotiated
	})?;

	{
		let mut queue = pending_frames.lock().expect("pending frames lock poisoned");
		queue.push_back(PendingVideoSurfaceFrame { surface_id: surface_id.to_string(), frame });
		while queue.len() > 2 {
			queue.pop_front();
		}
	}

	let _ = dispatch(RuntimeEvent::Workspace(WorkspaceRuntimeEvent::RedrawRequested));
...
fn video_surface_frame_from_sample(
	sample: &gst::Sample,
) -> BoxResult<WgpuVideoSurfaceNv12DmaBufFrame> {
	let caps = sample.caps().ok_or(GstVideoPlayerBridgeError::SampleMissingCaps)?;
	...
	let buffer = sample.buffer().ok_or(GstVideoPlayerBridgeError::SampleMissingBuffer)?;
...
fn dma_buf_plane_from_memory(
	...
	stride: i32,
	modifier: u64,
) -> BoxResult<WgpuVideoSurfaceDmaBufPlane> {
	if memory_index >= buffer.n_memory() {
		return Err(
			GstVideoPlayerBridgeError::MissingPlaneMemory {
				plane_index,
				memory_index,
	...
	let stride = u32::try_from(stride)
		.map_err(|_| GstVideoPlayerBridgeError::NegativePlaneStride { plane_index, stride })?;

	Ok(WgpuVideoSurfaceDmaBufPlane {
		fd: Arc::new(fd),
		offset: if buffer.n_memory() == 1 { offset as u64 } else { 0 },
		stride,
		modifier,
	})
}
...
#[cfg(target_os = "linux")]
fn duplicate_dmabuf_fd(memory: &gst::MemoryRef) -> BoxResult<OwnedFd> {
	...
	if let Some(drm_dumb) = memory.downcast_memory_ref::<gst_allocators::DRMDumbMemory>() {
		let exported = drm_dumb.export_dmabuf()?;
...
#[cfg(target_os = "linux")]
fn duplicate_raw_fd(fd: i32) -> BoxResult<OwnedFd> {
...
mod tests {
	...
	fn bridge_emits_pending_dmabuf_frames_for_local_video() {
		...
		let bridge = GstVideoPlayerBridge::new(Arc::new(|_| Ok(()))).expect("bridge should initialize");
