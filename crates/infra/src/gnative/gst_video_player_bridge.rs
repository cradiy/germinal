...
impl LinuxPlaybackRuntime {
	...
	fn open_file(&mut self, path: &str, surface_id: &str) -> Result<(), GstVideoPlayerBridgeError> {
		...
		let uri = gst::glib::filename_to_uri(canonical_path, None).map_err(|source| {
			GstVideoPlayerBridgeError::VideoPathToUri { path: path.to_string(), source }
		})?;
		...
		pipeline.set_state(gst::State::Paused).map_err(|source| {
			GstVideoPlayerBridgeError::PausePipeline { path: path.to_string(), source }
		})?;
	...
	fn seek(&self, position_us: u64) -> Result<(), GstVideoPlayerBridgeError> {
		...
		player
			...
			.map_err(|source| GstVideoPlayerBridgeError::SeekPlayback { position_us, source })?;
		Ok(())
	}
...
fn queue_video_sample(
	...
	if let Err(error) = dispatch(RuntimeEvent::Workspace(WorkspaceRuntimeEvent::RedrawRequested)) {
		warn!(
			surface_id,
			error = %error,
