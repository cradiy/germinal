    ...
    Dispatch: IRuntimeEventDispatcher,
{
    fn ensure_session_descriptor(
        ...
        response_rx
            .recv()
            .map_err(GNativeTunnelError::ResponseChannelClosed)?
    }
    ...
    fn begin_accept_session(&self, gshell_id: GShellId) -> Result<(), GNativeTunnelError> {
        let dispatcher = self
            ...
            .borrow()
            .clone()
            .ok_or(GNativeTunnelError::DispatcherNotConfigured)?;
    ...
    Dispatch: IRuntimeEventDispatcher,
{
    ...
    while let Ok(command) = command_rx.recv_async().await {
        match command {
            TunnelCommand::EnsureSessionDescriptor {
                gshell_id,
                protocol_version,
                response_tx,
            } => {
                let result =
                    ensure_session_descriptor_async(Rc::clone(&slots), gshell_id, protocol_version)
                        .await;
            ...
            TunnelCommand::BeginAcceptSession {
                ...
                match begin {
                    ...
                    BeginAccept::Failed(error) => {
                        dispatch_gnative_connection_failed(
                            gshell_id,
                            error.to_string(),
                            &dispatcher,
                        );
                    ...
                    BeginAccept::Start {
                        ...
                        spawn(async move {
                            ...
                            complete_accept(&slots, gshell_id, generation, result, &dispatcher)
                                .await;
                        })
                        .detach();
                    }
    ...
    Dispatch: IRuntimeEventDispatcher,
{
    let (mut stream, _) =
        listener
            .accept()
            .await
            .map_err(|source| GNativeTunnelError::AcceptConnection {
                gshell_id: descriptor.gshell_id.value(),
                source,
            })?;
    ...
    if let Some(mut writer) = slot.writer.take() {
        let result =
            writer
                .shutdown()
                .await
                .map_err(|source| GNativeTunnelError::ShutdownSession {
                    gshell_id: gshell_id.value(),
                    source,
                });
        slots.borrow_mut().insert(gshell_id, slot);
    ...
    W: AsyncWrite + Unpin + ?Sized,
{
    ...
    writer
        .flush()
        .await
        .map_err(|source| GNativeTunnelError::FlushMessage { source })?;
    Ok(())
}
...
fn take_buffered_app_message(
    ...
    serde_json::from_slice(&line)
        .map(Some)
        .map_err(GNativeTunnelError::DecodeMessage)
}
...
where
    Dispatch: IRuntimeEventDispatcher,
{
    if let Err(error) = dispatcher.dispatch(RuntimeEvent::GShell(GShellRuntimeEvent::ExitGNative {
        gshell_id,
    })) {
    ...
    Dispatch: IRuntimeEventDispatcher,
{
    ...
    if let Err(error) =
        dispatcher.dispatch(RuntimeEvent::GShell(GShellRuntimeEvent::GNativeConnected {
            accepted,
        }))
    {
...
mod tests {
    ...
    fn begin_accept_is_non_blocking_and_timeout_dispatches_failure() {
        ...
        tunnel
            ...
            .expect("accept should be queued");
        ...
        loop {
            ...
            assert!(
                ...
                "connection failure event should arrive"
            );
    ...
    fn present_frame_drops_stale_frame_before_snapshot_and_wakeup() {
        ...
        assert_eq!(
            presenter
                .surface_of(RenderTargetId::new(7))
                .unwrap()
                .latest_seq,
            Seq::new(2)
        );
    ...
    fn app_message_reader_rejects_a_buffer_over_the_limit() {
        ...
        assert!(matches!(
            error,
            GNativeTunnelError::MessageTooLarge { max_bytes: 8 }
        ));
    }
}
