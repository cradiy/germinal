...
impl KittyCommand {
	fn parse(bytes: &[u8]) -> Option<Self> {
		...
		for pair in control_bytes.split(|byte| *byte == b',').filter(|pair| !pair.is_empty()) {
	...
	fn u32(&self, key: u8, default: u32) -> Result<u32, KittyGraphicsError> {
		match self.control.get(&key) {
			Some(value) => value.parse().map_err(|_| KittyGraphicsError::InvalidControl),
			None => Ok(default),
		}
	}
...
enum DecoderState {
	...
	Escape,
	ApcPrefix,
	KittyApc { bytes: Vec<u8>, escape: bool, oversized: bool },
}
...
impl Default for KittyGraphicsStreamDecoder {
	fn default() -> Self { Self { state: DecoderState::Ground } }
}
	...
	pub(crate) fn feed(&mut self, input: &[u8]) -> Vec<KittyStreamEvent> {
		...
		for &byte in input {
			match &mut self.state {
				...
				DecoderState::KittyApc { bytes, escape, oversized } => {
...
impl KittyGraphicsState {
	...
	fn handle_inner(
		...
		if self.pending.is_some() {
			...
			let pending = self.pending.as_mut().expect("pending transmission must exist");
			...
			let pending = self.pending.take().expect("pending transmission must exist");
		...
		if matches!(action, 't' | 'T' | 'q') && command.u32(b'm', 0)? == 1 {
			...
			self.pending = Some(PendingTransmission { command, encoded_payload });
			return Ok(KittyCommandResult::default());
		}
	...
	fn place(
		...
		let image_id = command.u32(b'i', 0)?;
		let image_key = *self.image_ids.get(&image_id).ok_or(KittyGraphicsError::ImageNotFound)?;
		let placement_id = command.u32(b'p', 0)?;
		if command.u32(b'U', 0)? == 1 {
			self.virtual_images.insert(image_key);
			return Ok(KittyCommandResult {
	...
	fn insert_placement(
		...
		let image = self.images.get(&image_key).ok_or(KittyGraphicsError::ImageNotFound)?;
		let placement_id = command.u32(b'p', 0)?;
		...
		self.placements.insert(
			...
			KittyPlacement {
				...
				source_x_px,
				source_y_px,
				source_width_px: if source_width_px == 0 { max_width } else { source_width_px },
				source_height_px: if source_height_px == 0 { max_height } else { source_height_px },
				z_index: command.i32(b'z', 0)?,
			},
		);

		if command.u32(b'C', 0)? == 1 {
			Ok(KittyCursorMove { columns: 0, rows: 0 })
		} else {
	...
	fn remove_image(&mut self, key: u64) {
		...
		self.placements.retain(|_, placement| placement.image_key != key);
...
fn decode_raw(
	...
	let pixels = usize::try_from(width_px)
		.ok()
		.and_then(|width| usize::try_from(height_px).ok().and_then(|height| width.checked_mul(height)))
		.ok_or(KittyGraphicsError::ImageTooLarge)?;
	let expected = pixels.checked_mul(channels).ok_or(KittyGraphicsError::ImageTooLarge)?;
	...
	Ok(DecodedImage { width_px, height_px, rgba })
}
...
fn decode_png(bytes: &[u8]) -> Result<DecodedImage, KittyGraphicsError> {
	...
	let mut reader = decoder.read_info().map_err(|_| KittyGraphicsError::InvalidPayload)?;
	let width_px = reader.info().width;
	let height_px = reader.info().height;
	validate_dimensions(width_px, height_px)?;
	let output_size = reader.output_buffer_size().ok_or(KittyGraphicsError::ImageTooLarge)?;
	if output_size > MAX_DECODED_IMAGE_BYTES {
		return Err(KittyGraphicsError::ImageTooLarge);
	}
	let mut pixels = vec![0; output_size];
	let info = reader.next_frame(&mut pixels).map_err(|_| KittyGraphicsError::InvalidPayload)?;
...
mod tests {
	...
	fn decoder_preserves_text_and_extracts_split_kitty_apc() {
		...
		assert_eq!(decoder.feed(b"before\x1b_"), vec![KittyStreamEvent::Bytes(b"before".to_vec())]);
	...
	fn transmits_and_places_raw_rgba() {
		...
		let result = graphics.handle(command("a=T,f=32,s=1,v=1,i=7,p=2,c=3,r=4", payload.as_bytes()), (5, 6));

		assert_eq!(result.response, Some(b"\x1b_Gi=7,p=2;OK\x1b\\".to_vec()));
		assert_eq!(result.cursor_move, Some(KittyCursorMove { columns: 3, rows: 4 }));
	...
	fn joins_chunked_base64_before_decoding() {
		...
		assert_eq!((graphics.snapshots(&[])[0].x_cell, graphics.snapshots(&[])[0].y_cell), (2, 3));
	}
	...
	fn yazi_quiet_upload_does_not_inject_a_response_into_input() {
		...
		let result = graphics.handle(
			command(
				"q=2,a=T,C=1,U=1,f=32,s=1,v=1,i=7",
				payload.as_bytes(),
			),
			(0, 0),
		);
	...
	fn virtual_placement_uses_connected_placeholder_bounds() {
		...
		let placeholders = [
			KittyPlaceholderCell { image_id: 7, x_cell: 4, y_cell: 3 },
			KittyPlaceholderCell { image_id: 7, x_cell: 5, y_cell: 3 },
			KittyPlaceholderCell { image_id: 7, x_cell: 4, y_cell: 4 },
			KittyPlaceholderCell { image_id: 7, x_cell: 5, y_cell: 4 },
		];
