...
impl TerminalGridPixelRect {
	...
	pub fn right_px(self) -> u32 { self.x_px.saturating_add(self.width_px) }

	pub fn bottom_px(self) -> u32 { self.y_px.saturating_add(self.height_px) }
}
...
impl TerminalRenderViewport {
	pub const fn new(
		...
		Self { cell_size, origin, grid_size }
	}

	pub const fn cell_size(self) -> TerminalCellSize { self.cell_size }

	pub const fn origin(self) -> TerminalPadding { self.origin }

	pub const fn grid_size(self) -> TerminalGridSize { self.grid_size }

	pub const fn origin_x_px(self) -> u32 { self.origin.x_px() }

	pub const fn origin_y_px(self) -> u32 { self.origin.y_px() }
