...
impl Label {
    ...
    fn into_div(self) -> Div {
        ...
        match self.secondary {
            Some(secondary) => h_flex()
                .gap_1()
                .child(primary)
                .child(div().text_color(rgb(130, 140, 158)).child(secondary)),
            None => primary,
        }
    }
}
...
impl InputState {
    ...
    pub fn clean_on_escape_enabled(&self) -> bool {
...
mod tests {
    ...
    fn input_state_renders_cursor_when_focused() {
        ...
        assert_eq!(
            compiled.cursor,
            Some(germinal_gnative_ui::GridPoint { x: 5, y: 0 })
        );
        assert!(compiled.commands.iter().any(|command| matches!(
            command,
