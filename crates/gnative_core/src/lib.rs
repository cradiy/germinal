...
pub const fn rgb(red: u8, green: u8, blue: u8) -> RgbColorDto {
    RgbColorDto::new(red, green, blue)
}
...
impl UiTree {
    pub fn new(root: impl IntoElementNode) -> Self {
        Self {
...
impl IntoDivChild for Element {
    fn into_child(self, _inherited_style: TextStyleDto) -> Element {
        self
    }
}
...
pub fn uniform_list() -> UniformList {
    div()
}

pub fn video(id: impl Into<String>) -> Video {
    Video::new(id)
}
...
impl Div {
    ...
    pub fn h_flex(self) -> Self {
        self.flex_row()
    }

    pub fn v_flex(self) -> Self {
        self.flex_col()
    }
    ...
    for paint in &node.paints {
        match paint {
            LayoutPaint::FillRect { rect, color } => {
                commands.push(RenderCommandDto::PixelFillRect {
                    x_px: rect.x_px,
                    y_px: rect.y_px,
            ...
            LayoutPaint::VideoSurface { id, rect } => {
                commands.push(RenderCommandDto::VideoSurface {
                    id: id.clone(),
                    x_px: rect.x_px,
                    y_px: rect.y_px,
                    width_px: rect.width_px,
...
fn layout_div(
    ...
    if let Some(color) = div.background {
        node.paints.push(LayoutPaint::FillRect {
            rect: rect.pixel_rect(),
            color,
        });
    }
    ...
    match div.display {
        Display::Block => {
            for child in &div.children {
                node.children
                    .push(layout_element(child, content_rect, state, current_style));
            }
        }
...
fn layout_absolute_div(div: &Div, rect: Rect) -> LayoutNode {
    ...
    let width_px = div
        .width
        .map(pixels_of)
        .unwrap_or(u32::from(rect.width) * CELL_WIDTH_PX);
    let height_px = div
        .height
        .map(pixels_of)
        .unwrap_or(u32::from(rect.height) * CELL_HEIGHT_PX);
...
fn layout_flex_children(
    ...
    let flow_children: Vec<_> = div
        .children
        .iter()
        .filter(|child| !is_absolute(child))
        .collect();
    let absolute_children: Vec<_> = div
        .children
        .iter()
        .filter(|child| is_absolute(child))
        .collect();
    ...
    let gap_total = div
        .gap
        .saturating_mul(flow_children.len().saturating_sub(1) as u16);
    ...
    let fixed_total: u16 = flow_children
        ...
        .sum();
    let flex_total: u32 = flow_children
        .iter()
        .map(|child| u32::from(child_flex_grow(child)))
        .sum();
...
fn layout_input(
    ...
    if input.focused {
        let cursor_prefix = clip_line(
            &input.value[..input.cursor_byte_index.min(input.value.len())],
            rect.width,
        );
        let cursor_x = rect
            .x
...
fn layout_video(video: &Video, rect: Rect) -> LayoutNode {
    ...
    node.paints.push(LayoutPaint::VideoSurface {
        id: video.id.clone(),
        rect: rect.pixel_rect(),
    });
    node
...
fn pixels_of(length: Pixels) -> u32 {
    length.0.max(0.0).round() as u32
}

fn cells_of(length: Pixels) -> i32 {
    length.0.max(0.0).round() as i32
}

fn child_fixed_extent(child: &Element, axis: Axis) -> u16 {
    intrinsic_extent(child, axis)
}
...
fn intrinsic_div_extent(div: &Div, axis: Axis) -> u16 {
    ...
    let content_extent = match (div.display, axis) {
        (Display::Block, Axis::Horizontal) | (Display::FlexCol, Axis::Horizontal) => flow_children
            .iter()
        ...
        (Display::FlexRow, Axis::Horizontal) => flow_children
            ...
            .sum::<u16>()
            .saturating_add(gap_total),
        (Display::FlexRow, Axis::Vertical) => flow_children
            .iter()
            .map(|child| intrinsic_extent(child, axis))
            .max()
            .unwrap_or(0),
...
mod tests {
    ...
    fn flex_row_measures_text_children_by_intrinsic_width() {
        let tree = UiTree::new(
            h_flex()
                .gap_1()
                .child(div().child("alpha"))
                .child(div().child("beta")),
        );
    ...
    fn flex_column_measures_nested_container_height() {
        let tree = UiTree::new(
            v_flex()
                .child("header")
                .child(v_flex().gap_1().child("task one").child("task two")),
        );
    ...
    fn layout_tree_preserves_structured_fill_node() {
        let tree = UiTree::new(
            h_flex()
                .child("left")
                .child(div().w(px(1.0)).bg(rgba(1, 2, 3, 255)))
                .child("right"),
        );
    ...
    fn styled_text_input_preserves_cursor_and_color() {
        let tree = UiTree::new(div().child(styled_text_input(
            "xyz",
            true,
            TextStyleDto {
