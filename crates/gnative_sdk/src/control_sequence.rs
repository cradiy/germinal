...
impl GNativeTunnelEnv {
    pub fn from_env() -> GNativeSdkResult<Self> {
        let endpoint = std::env::var(GNATIVE_TUNNEL_ENDPOINT_ENV).map_err(|_| {
            GNativeSdkError::MissingEnv {
                name: GNATIVE_TUNNEL_ENDPOINT_ENV,
            }
        })?;
        let token =
            std::env::var(GNATIVE_TUNNEL_TOKEN_ENV).map_err(|_| GNativeSdkError::MissingEnv {
                name: GNATIVE_TUNNEL_TOKEN_ENV,
            })?;
        let protocol_version = std::env::var(GNATIVE_TUNNEL_PROTOCOL_VERSION_ENV)
            .map_err(|_| GNativeSdkError::MissingEnv {
...
mod tests {
    ...
    fn encodes_enter_control_sequence() {
        ...
        assert_eq!(
            tunnel_env.enter_control_sequence(),
    ...
    fn writes_enter_control_sequence_to_writer() {
        ...
        write_enter_control_sequence(&mut buffer, &tunnel_env)
            .expect("writer should accept sequence");
