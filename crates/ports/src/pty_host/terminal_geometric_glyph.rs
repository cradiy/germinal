...
impl TerminalGeometricGlyphRect {
	pub const fn new(x_px: u32, y_px: u32, width_px: u32, height_px: u32) -> Self {
		Self { x_px, y_px, width_px, height_px }
	}
...
impl BoxDrawingGlyph {
	...
	fn orthogonal(left: StrokeStyle, right: StrokeStyle, up: StrokeStyle, down: StrokeStyle) -> Self {
		Self::Orthogonal(OrthogonalBoxGlyph { left, right, up, down })
	}
...
impl TerminalRasterGlyph {
	pub const fn new(columns: u8, rows: u8, mask: u64) -> Self { Self { columns, rows, mask } }
	...
	fn pixel_rects_for_size(self, width_px: u32, height_px: u32) -> Vec<TerminalGeometricGlyphRect> {
...
fn cell_mask(columns: u32, column: u32, row: u32) -> u64 { 1u64 << (row * columns + column) }
