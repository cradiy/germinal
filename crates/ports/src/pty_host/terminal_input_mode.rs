...
impl TerminalInputModes {
	...
	pub const fn app_cursor(self) -> bool { self.0 & APP_CURSOR != 0 }

	pub const fn bracketed_paste(self) -> bool { self.0 & BRACKETED_PASTE != 0 }

	pub const fn focus_in_out(self) -> bool { self.0 & FOCUS_IN_OUT != 0 }

	pub const fn sgr_mouse(self) -> bool { self.0 & SGR_MOUSE != 0 }

	pub const fn mouse_report_click(self) -> bool { self.0 & MOUSE_REPORT_CLICK != 0 }

	pub const fn mouse_drag(self) -> bool { self.0 & MOUSE_DRAG != 0 }

	pub const fn mouse_motion(self) -> bool { self.0 & MOUSE_MOTION != 0 }
