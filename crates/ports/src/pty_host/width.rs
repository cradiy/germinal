...
/// placement.
pub fn terminal_char_cell_width(c: char) -> u32 {
    c.width().unwrap_or(0).min(2) as u32
}

pub fn terminal_char_cell_advance(c: char) -> u32 {
    terminal_char_cell_width(c).max(1)
}
