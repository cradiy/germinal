    ...
    T: TerminalSnapshotProvider,
{
    pub fn new(
        ...
        Self {
            presenter,
            surface_snapshot_provider,
            renderer_backend,
            terminal_snapshot_provider,
        }
    }
    ...
    pub fn present_frame(&self, frame: &BuiltFramePlan) -> SurfacePresentationResult {
        ...
        self.terminal_snapshot_provider
            .clear_damage_up_to(frame.target_id, frame.seq);

        let surface_snapshot = self
            .surface_snapshot_provider
            .surface_snapshot_of(frame.target_id);
    ...
    pub fn renderer_backend(&self) -> &R {
        &self.renderer_backend
    }

    pub fn surface_snapshot_provider(&self) -> &S {
        &self.surface_snapshot_provider
    }

    pub fn terminal_snapshot_provider(&self) -> &T {
        &self.terminal_snapshot_provider
    }
}
...
mod tests {
    ...
    impl FramePlanPresenter for RejectingPresenter {
        fn present(&self, _frame: &BuiltFramePlan) -> bool {
            false
        }
    }
    ...
    impl TerminalSnapshotProvider for TestTerminalSnapshotProvider {
        fn snapshot_of(&self, _render_target_id: RenderTargetId) -> Option<TerminalSnapshot> {
            None
        }
        ...
        fn clear_damage_up_to(&self, render_target_id: RenderTargetId, presented_seq: Seq) {
            self.cleared
                .borrow_mut()
                .push((render_target_id, presented_seq));
        }
    }
    ...
    fn presents_frame_clears_damage_and_renders_surface_snapshot() {
        ...
        let frame = BuiltFramePlan {
            target_id,
            seq,
            commands: vec![RenderCommandDto::TextRun {
                x: 0,
                y: 0,
                text: "hello".to_string(),
            }],
