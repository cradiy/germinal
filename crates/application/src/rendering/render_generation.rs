...
impl RenderGenerationState {
	...
	pub fn latest_input_seq(&self) -> Seq { self.latest_input_seq }

	pub fn in_flight_seq(&self) -> Option<Seq> { self.in_flight_seq }

	pub fn ready_seq(&self) -> Option<Seq> { self.ready_seq }

	pub fn presented_seq(&self) -> Option<Seq> { self.presented_seq }
	...
	pub fn can_start_build(&self) -> bool { self.dirty && self.in_flight_seq.is_none() }
	...
	pub fn complete_build(&mut self, seq: Seq) -> BuildCompletion {
		...
		if self.ready_seq.map_or_else(|| true, |ready| seq > ready) {
			...
			if self.dirty { BuildCompletion::ReadyAndNeedsRebuild } else { BuildCompletion::Ready }
		} else {
	...
	pub fn mark_presented(&mut self, seq: Seq) -> bool {
		...
		if self.presented_seq.map_or_else(|| false, |presented| seq <= presented) {
			return false;
		}
...
mod tests {
	...
	fn completing_build_reports_need_rebuild_when_newer_input_arrived() {
		...
		assert_eq!(state.complete_build(Seq::new(1)), BuildCompletion::ReadyAndNeedsRebuild);
