...
impl RenderScheduler {
    ...
    pub fn register_target(&mut self, target_id: RenderTargetId) {
        self.targets
            .entry(target_id)
            .or_insert_with(|| RenderGenerationState::new(Seq::ZERO));
    }
    ...
    pub fn mark_input_updated(
        ...
        state.start_build().map(|build_seq| BuildRequest {
            target_id,
            seq: build_seq,
        })
    }
    ...
    pub fn complete_build(&mut self, target_id: RenderTargetId, seq: Seq) -> BuildResult {
        ...
        match completion {
            ...
            BuildCompletion::ReadyAndNeedsRebuild => {
                let next_request = state.start_build().map(|next_seq| BuildRequest {
                    target_id,
                    seq: next_seq,
                });
            ...
            BuildCompletion::Stale => BuildResult {
                accepted: false,
                ready: None,
                next_request: None,
            },
...
mod tests {
    ...
    fn complete_build_creates_next_request_for_latest_seq() {
        ...
        assert_eq!(
            ...
            BuildResult {
                accepted: true,
                ready: Some(ReadyFrame {
                    target_id,
                    seq: Seq::new(1)
                }),
                next_request: Some(BuildRequest {
                    target_id,
                    seq: Seq::new(5)
                }),
            }
