...
{
	pub fn new(executor: E) -> Self { Self { scheduler: RenderScheduler::new(), executor } }
	...
	fn submit(&self, request: BuildRequest) { self.executor.submit(request.into_task()); }
}
...
impl FrameBuiltResult {
	pub fn ready_frame(&self) -> Option<ReadyFrame> { self.result.ready }
}
...
mod tests {
	...
	fn frame(target_id: RenderTargetId, seq: Seq) -> BuiltFramePlan {
		BuiltFramePlan { target_id, seq, commands: vec![RenderCommandDto::Clear] }
	}
	...
	fn input_update_submits_build_task() {
		...
		assert_eq!(
			result,
			InputUpdateResult::TaskSubmitted(BuildRequest { target_id, seq: Seq::new(1) })
		);

		assert_eq!(pipeline.executor.submitted(), vec![BuildFramePlanTask {
			target_id,
			seq: Seq::new(1)
		}]);
	}
	...
	fn frame_built_with_newer_dirty_input_submits_latest_next_task() {
		...
		assert_eq!(built.ready_frame(), Some(ReadyFrame { target_id, seq: Seq::new(1) }));

		assert_eq!(built.result.next_request, Some(BuildRequest { target_id, seq: Seq::new(5) }));

		assert_eq!(pipeline.executor.submitted(), vec![
			BuildFramePlanTask { target_id, seq: Seq::new(1) },
			BuildFramePlanTask { target_id, seq: Seq::new(5) },
		]);
	}
