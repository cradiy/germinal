...
use germinal_domain::{
    ...
    pty_host::{
        entity::pty_host::PtyHost, pty_host_id::PtyHostId, terminal_size::TerminalGridSize,
    },
};
use germinal_ports::{
...
impl GShellServiceState {
    ...
    pub fn pty_service_state(&self) -> &PtyServiceState {
        &self.pty_service_state
    }

    pub fn gnative_service_state(&self) -> &GNativeServiceState {
        &self.gnative_service_state
    }
    ...
    fn create_pty_host(&self, gshell_id: GShellId, grid_size: TerminalGridSize) -> PtyHostId {
        ...
        self.pty_hosts
            .borrow_mut()
            .insert(pty_host_id, PtyHost::new(grid_size));
        self.shells
            .borrow_mut()
            .insert(gshell_id, GShell::new(pty_host_id));
        pty_host_id
    }
...
where
    Deps: AsRef<GShellServiceState> + IPtyService + IGNativeService,
{
    fn ensure_gshell(
        &self,
        gshell_id: GShellId,
